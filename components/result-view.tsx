...
export function ResultView() {
  ...
  return <>
    <main className="scanner-page"><div className="container">
      {result.isDemoData && <div className="demo-banner">DATA DEMO — hasil ini berasal dari TRACE Demo Dataset dan bukan statistik dunia nyata.</div>}
      ...
      <div className="result-grid">
        ...
        <section className="panel"><span className="eyebrow">TINDAKAN</span><h2>Prioritas perlindungan</h2>{result.recommendations.slice(0, 4).map((item) => <div className="recommendation" key={item.id}><h3>{item.title}</h3><p>{item.description}</p></div>)}<Link className="button button-primary" href="/register" style={{ marginTop: 20 }}>Simpan & Pantau →</Link></section>
      </div>
